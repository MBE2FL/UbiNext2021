	...
	, m_nRows(nRows)
{
	...
	{
		CalculateUVs();
		_vertices[0] = -(m_width / 2.0f);
		_vertices[1] = -(m_height / 2.0f);
		_vertices[2] = m_width / 2.0f;
		_vertices[3] = -(m_height / 2.0f);
		_vertices[4] = m_width / 2.0f;
		_vertices[5] = m_height / 2.0f;
		_vertices[6] = -(m_width / 2.0f);
		_vertices[7] = m_height / 2.0f;
	}
}
...
{            
    ...
    for (unsigned int i = 0; i < 8; i += 2)
    {
        ...
        glVertex2f(_vertices[i], _vertices[i+1]);
    }
    glEnd();
    glPopMatrix();
    glDisable(GL_BLEND);
